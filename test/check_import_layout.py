...
assert discover(ROOT) == ['RunecraftHelper.csproj'], discover(ROOT)
for path in ROOT.rglob('*.csproj'):
    parts = path.relative_to(ROOT).parts
    if any(part in ('obj', 'bin') for part in parts):
        continue
    assert path == ROOT / 'RunecraftHelper.csproj' or parts[0] == 'test', path
project = ET.parse(ROOT / 'RunecraftHelper.csproj').getroot()
assert project.findtext('./PropertyGroup/AssemblyName') == 'RunecraftHelper'
...
asset_patterns = ['icons/**/*.png', 'Localization/**/*.json', 'expedition2_recipes.json', 'relic_mods_en.json']
...
with tempfile.TemporaryDirectory(prefix='runecrafthelper-import-check-') as temporary:
    repo = Path(temporary)
    ...
    assert discover(repo) == ['RunecraftHelper.csproj']
    ...
    extra.unlink()
    output = subprocess.check_output([DOTNET, 'msbuild', str(repo / 'RunecraftHelper.csproj'),
    ...
    assert evaluated['Properties'] == {'AssemblyName': 'RunecraftHelper', 'RootNamespace': 'RunecraftHelper'}
