...
`RunecraftHelper.csproj` and production source live at repository root. Tests are in
`test/RunecraftHelper.Tests.csproj`; all future auxiliary projects/tools must live under
`test/` (tools under `test/tools/`). Assembly name and root namespace remain
`RunecraftHelper` and are now explicit.

Production source remains at root. JSON, `icons/`, and `Localization/` retain
their DLL-relative paths and contents. The old tests-only Compile removal is
replaced by exclusions covering all SDK item kinds and nested build artifacts.
...
"$DOTNET" restore RunecraftHelper.csproj --no-dependencies -p:EnableWindowsTargeting=true -p:RuntimeIdentifier=win-x64
"$DOTNET" build RunecraftHelper.csproj -c Release --no-restore -p:EnableWindowsTargeting=true -p:RuntimeIdentifier=win-x64 -p:BuildProjectReferences=false
python3 test/check_import_layout.py
git diff --check
```

Restore excludes dependencies and build disables project-reference builds, so the
host is only read. Output: `bin/Release/net10.0-windows/win-x64/RunecraftHelper.dll`.
